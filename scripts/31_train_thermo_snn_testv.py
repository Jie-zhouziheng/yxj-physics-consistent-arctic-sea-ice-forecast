# scripts/31_train_thermo_snn_testv.py
...
def masked_mse_torch(pred: torch.Tensor, target: torch.Tensor, wmask: torch.Tensor, denom: float):
    """
    pred/target: (B, K, H, W) or (B, 1, H, W)
...
@torch.no_grad()
def evaluate_first_step(model, loader, device, wmask, denom, sat_lookup: SatScalarLookup):
    """
    Minimal multi-step eval:
      If model outputs K steps, evaluate only step-1 (k=0).
      Keeps your old metric semantics.
    """
    model.eval()
    maes, rmses = [], []

    for x, y, meta in loader:
        assert x.shape[0] == 1, "evaluate_first_step() assumes batch_size=1"
...
def main():
    ...
    for ep in range(1, epochs + 1):
        ...
        for x, y, meta in train_loader:
            x = x.unsqueeze(2).float().to(device)  # (B,T,1,H,W)
            y = y.float().to(device)               # (B,K,H,W)  <-- NEW
            ...
            pred = model(x, context=context)  # (B,K,H,W)  <-- NEW
        ...
        val_mae, val_rmse = evaluate_first_step(model, val_loader, device, wmask, denom, sat_lookup)
        print(f"[Epoch {ep:02d}] Train masked-MSE={train_mse:.6f} | Val(step1) MAE={val_mae:.4f} RMSE={val_rmse:.4f}")
    ...
    test_mae, test_rmse = evaluate_first_step(model, test_loader, device, wmask, denom, sat_lookup)

    print("\n=== SNN Phase-1 (Test, masked, step1) ===")
    ...
    append_result_row(out_csv, {
        "model": "SNN_phase1_thermo_sat_directK",
        "split": "test",
