...
def load_runs(tag: str):
    paths = sorted((Path("scripts/results/fig2_json")).glob(f"fig2_{tag}_seed*.json"))
    if not paths:
        raise FileNotFoundError(f"No json for tag={tag}")
...
def plot_one(ax, lead, mu, sd, label):
