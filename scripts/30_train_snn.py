...
from src.models.snn_baseline import SNNBaseline
...
def main():
    ...
    ds_train = SICWindowDataset(index_train, input_window=input_window, lead_time=lead_time, cache_in_memory=True)
    ds_val   = SICWindowDataset(index_val,   input_window=input_window, lead_time=lead_time, cache_in_memory=True)
    ds_test  = SICWindowDataset(index_test,  input_window=input_window, lead_time=lead_time, cache_in_memory=True)
    ...
    # ---- DataLoader kwargs: only pass args that are valid ----
    dl_kwargs = dict(
