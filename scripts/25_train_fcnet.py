...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/fcnet_base.yaml")
    parser.add_argument("--seed", type=int, default=None)
    args = parser.parse_args()

    cfg = load_config(args.config)

    # A protocol: fixed lead_time=1, out_steps=6 (readable from cfg)
    lead_time = 1
    ...
    tag = f"fcnet_K{out_steps}_seed{seed}"
    ...
    hidden_channels = int(get(cfg, "model.hidden_channels", 64))
    num_blocks = int(get(cfg, "model.num_blocks", 4))
    ...
    ckpt_name = f"FCNet_K{out_steps}_seed{seed}.pt"
    ...
    # save per-step rows
    out_csv = res_dir / f"models_test_fcnet_K{out_steps}.csv"
