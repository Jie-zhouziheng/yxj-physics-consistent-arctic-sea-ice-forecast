...
from src.models.convlstm import EncoderDecoderConvLSTM
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/convlstm_base.yaml")
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--tf_ratio", type=float, default=0.0, help="teacher forcing ratio in [0,1]")
    ...
    tag = f"convlstm_K{out_steps_tmp}_seed{seed}"
    ...
    ksz = int(get(cfg, "model.kernel_size", 3))
    ...
    # model
    model = EncoderDecoderConvLSTM(in_ch=1, hid_ch=hid_ch, k=ksz, out_steps=out_steps, use_sigmoid=True).to(device)
    opt = torch.optim.Adam(model.parameters(), lr=lr)

    ckpt_name = f"ConvLSTM_lead{lead_time}_K{out_steps}_seed{seed}.pt"
    ...
    for ep in range(1, epochs + 1):
        ...
        for x, y, meta in train_loader:
            ...
            pred = model(x, y_teacher=y, teacher_forcing_ratio=float(args.tf_ratio))
    ...
    out_json = fig2_dir / f"fig2_convlstm_seed{seed}.json"
    payload = {"model":"convlstm","seed":seed,"lead":list(range(1,out_steps+1)),
