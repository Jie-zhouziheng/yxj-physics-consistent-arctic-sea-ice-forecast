...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/predrnn_base.yaml")
    ...
    tag = f"predrnn_K{out_steps}_seed{seed}"
    ...
    batch_size = int(get(cfg, "train.batch_size", 1))  # recommend 1
    lr = float(get(cfg, "train.lr", 1e-3))

    hidden_channels = int(get(cfg, "model.hidden_channels", 32))
    num_layers = int(get(cfg, "model.num_layers", 2))
    kernel_size = int(get(cfg, "model.kernel_size", 3))
    downscale = int(get(cfg, "model.downscale", 1))
    ...
    ckpt_name = f"PredRNN_K{out_steps}_seed{seed}.pt"
    ...
    # save: per-step rows => Lead=1..6
    out_csv = res_dir / f"models_test_predrnn_K{out_steps}.csv"
    for k in range(out_steps):
        append_result_row(out_csv, {
            "model": "PredRNN",
            "split": "test",
            "input_window": input_window,
            "lead_time": k + 1,          # Lead=step index
            "out_steps": out_steps,
            "mask": mask_name,
            "seed": seed,
